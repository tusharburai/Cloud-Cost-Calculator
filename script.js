...
const pricing = {
  ...
  gcp: { hot: 0.02, cool: 0.01, archive: 0.0015, dataOut: 0.12 },
};
...
function calculateMainCosts() {
  ...
  const computeHours =
    parseFloat(document.getElementById("computeHours")?.value) || 730;
  const computeRate =
    parseFloat(document.getElementById("computeRate")?.value) || 0.1;
...
function updateTable() {
  const storageGB =
    parseFloat(document.getElementById("storage")?.value) || 100;
...
function resetCalculator() {
  ...
  document.getElementById("computeRate").value = 0.1;
...
function updateRealTimeChart() {
  const storage =
    parseFloat(document.getElementById("storageInput").value) || 0;
  const multiplier = durationMultiplier[selectedDuration] || 1;
  ...
  // Update chart
  realTimeChart.data.datasets[0].data = [awsCost, azureCost, gcpCost];
